...
def index():
    user = get_user()
    per_page = 6  # Número de películas por página
    page = request.args.get('page', 1, type=int)  # Obtener el número de página de la URL
    ...
    with db.get_db() as conn:
        ...
        try:
            ...
            total_pages = ceil(total_movies / per_page)  # Calcular el total de páginas
            ...
            recent_movies = []  # Si la tabla no existe, simplemente se devuelve una lista vacía
            total_pages = 1  # Asegúrate de que haya al menos una página

    return render_template('index.html', movies=recent_movies, user=user, total_pages=total_pages, current_page=page)
    ...
    if request.method == 'POST':
        ...
        if not nickname or not email:
            ...
            success = auth_controller.update_user_profile(
                user['id'], nickname, email, new_password if new_password else None
            )
            if success:
...
def search():
    ...
    if query:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            # Solicitud AJAX para búsqueda en tiempo real
    ...
    # Página de búsqueda inicial con recomendaciones aleatorias
